...
def import_pull_requests(organization_key, repository_key, source_pull_requests):
    try:
        with db.orm_session() as session:
            return pull_requests.import_pull_requests(session, organization_key, repository_key, source_pull_requests)
    except SQLAlchemyError as exc:
