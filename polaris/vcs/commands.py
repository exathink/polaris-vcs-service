...
def sync_pull_requests(repository_key, connector_key, source_repo_id, updated_after=None):
    ...
    if connector:
        yield polaris.vcs.db.api.sync_pull_requests(
            connector.organization_key,
            repository_key,
            connector.fetch_pull_requests_from_source(source_repo_id, updated_after)
        )
